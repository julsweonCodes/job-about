...
interface EmployerJobPostEditViewProps {
  jobData: JobPostData;
...
const JobDescription: React.FC<{
  ...
  // Gemini API 응답이 있는지 확인
  const hasGeminiResponse = geminiRes && geminiRes.length > 0;
    ...
    <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100">
      <div className="mb-6">
        <div className="flex items-center justify-between mb-3">
          ...
          {showEditButtons && onEdit && editableSections.includes("skillsAndStyles") && (
            <button
              onClick={() =>
...
const ActionButtons: React.FC<{
  ...
  const renderButton = (type: "publish" | "saveEdit") => {
    const config = {
      publish: {
...
const EmployerJobPostEditView: React.FC<EmployerJobPostEditViewProps> = ({
  jobData,
  onEdit,
  onPublish,
  onSaveEdit,
...
export default EmployerJobPostEditView;
