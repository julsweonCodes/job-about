...
import { calculateJobRecommendationScores, getCompatibilityLevel } from "@/app/lib/work-style-matcher";
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!user.personality_profile_id) {
      ...
      }, 200, "No personality profile found. Please complete the quiz first.");
    }
    ...
    console.log(`추천 완료: ${filteredRecommendations.length}개 공고 추천 (최고 점수: ${filteredRecommendations[0]?.matchScore || 0})`);
    ...
    }, 200, "Job recommendations generated successfully.");
  } catch (error) {
